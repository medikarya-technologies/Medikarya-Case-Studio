...
export default function ReviewerDashboard() {
  ...
  const fetchCases = useCallback(async (retryCount = 0) => {
    ...
    try {
      ...
      setCases(allCases.filter(c => c.status === 'submitted'));
    } catch (e) {
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600 mt-2">Review and provide feedback on submitted cases</p>
      </div>
      ...
      {cases.length === 0 ? (
        <Card className="text-center py-12 shadow-sm">
          <CardContent>
            ...
            <p className="text-lg text-gray-600">No cases to review at the moment</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
